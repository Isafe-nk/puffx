...
- **Desktop home** — the landing surface: a grid of **app tiles** on the cream canvas. Each tile is
  an app and shows a live status line (Learn → "12 of 68 read", ETF Drag → last scenario,
  Tracker → net worth). Replaces the marketing-style landing/hub.
- **Dock** — a persistent rail, always visible: one icon per app + home. Hop app→app directly;
  never feel lost. (Evolves the current SideNav.) Below `lg` it collapses to a drawer that closes
  on route change / Escape / overlay tap.
- **Apps open full-focus** — an app fills the workspace beside the dock. **No draggable windows,
  no tabs, no window management** — that's PostHog's gimmick layer, not ours.
- **Depth panel (tree)** — only inside apps that have depth. Learn gets a module → lesson tree
  panel; single-screen apps (visualizers, glossary) don't show one.
- **Keyboard-navigable** shell (dock and tree: tab / arrows / enter). Command palette: deferred —
  unnecessary at 4 apps.
...
New shell components this direction introduces: **`Desktop`** (tile grid home), **`AppTile`**
(icon + name + live status line), **`Dock`** (persistent rail), **`TreePanel`** (Learn's
module→lesson nav).
...
1. **Mock first (gate).** Static mocks of the desktop home + Learn open in the shell, in the Dragon
   palette. **KY approves before any code.** Palette/type values above finalize here.
...
4. **Shell.** Build `Dock` (from SideNav), `Desktop` + `AppTile` home (replaces VisualizerHub as
   the landing), full-focus app routing. Mobile drawer behaviour preserved.
