...
export default function Desktop() {
  ...
  const resumePath = next ? lessonPath(next.module, next.lessonId) : `/learn/${LEARN_MODULES[0].slug}`;
  ...
  return (
    <div className="os-desk-layer h-full min-h-0 flex flex-col lg:grid lg:grid-cols-[auto_1fr_auto] overflow-y-auto lg:overflow-hidden">
      ...
      >
        {apps.map((app) => (
          <AppIcon key={app.id} app={app} onOpen={() => openApp(app.id)} />
        ))}
      </nav>
      ...
      <aside className="order-3 lg:order-none w-full lg:w-[314px] flex flex-col gap-3.5 px-4 pb-6 lg:py-[26px] lg:px-[22px] lg:overflow-y-auto">
        ...
        <Widget title="Continue learning">
          <div className="os-well rounded-[2px] px-3.5 py-3">
            <div className="text-[31px] font-extrabold tracking-[-0.02em] leading-none text-ink">
              {pct}<small className="text-[13px] font-semibold text-faint">%</small>
            </div>
            <div className="text-[11.5px] text-mute mt-1.5">{read} of {TOTAL_LESSONS} lessons read</div>
          </div>
          {next && (
        ...
        <Widget title="Saved plan">
          <div className="os-well rounded-[2px] px-3.5 py-3">
            <div className="text-[31px] font-extrabold tracking-[-0.02em] leading-none text-ink font-mono">RM 1.82M</div>
            <div className="text-[11.5px] text-mute mt-1.5">projected net worth at 60 · on track</div>
          </div>
