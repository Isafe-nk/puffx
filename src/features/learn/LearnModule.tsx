      ...
      <div className="max-w-3xl mx-auto px-6 lg:px-8 pb-20">
        ...
        <div className="pt-12 lg:pt-16">
          ...
          <p className="mt-3 text-[12px] text-[#A2A3A5] font-mono">
            ...
            {readHere > 0 && <span className="text-[#0EB35B]"> · {readHere} of {mod.lessons.length} read</span>}
          </p>
        </div>
          ...
          {mod.lessons.map((l) => {
            ...
            return (
              <li key={l.id} className="border-b border-[#EEEEEE]">
                ...
                >
                  <span
                    className={`shrink-0 w-4 h-4 rounded-full flex items-center justify-center ${read ? 'bg-[#0EB35B]' : 'border border-[#D0D1D2]'}`}
